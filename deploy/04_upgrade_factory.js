...
module.exports = async ({ getNamedAccounts, deployments }) => {
    ...
    console.log("读取到的工厂代理合约地址:", proxyAddress);
    console.log("读取到的工厂实现合约地址:", implAddress);

    // 开始升级
    console.log("开始升级工厂合约...");
    const AuctionFactoryV2 = await ethers.getContractFactory("AuctionFactoryV2");

    const auctionFactoryV2Proxy = await upgrades.upgradeProxy(proxyAddress, AuctionFactoryV2, {
        initializer: "initialize",
    });

    console.log("合约升级完成");
    const proxyAddressV2 = auctionFactoryV2Proxy.target

    console.log("代理工厂合约地址:", proxyAddressV2);
    const newImplAddress = await upgrades.erc1967.getImplementationAddress(
        proxyAddressV2
    );
    console.log("新的工厂实现合约地址:", newImplAddress);

    // 保存json文件
    const deploymentsDir = path.join(
        __dirname,
        "./.cache",
        "proxyAuctionFactoryV2.json"
    );
    fs.writeFileSync(
        deploymentsDir,
        JSON.stringify({
